...
const copyStringToSnakeCaseCB = (str) => str.toLowerCase().split(" ").join("_");

console.log(copyAndChangeArr(lotr, copyToCamelCaseCB));
console.log(copyAndChangeArr(lotr, copyStringToSnakeCaseCB));
...
const mappedColors = colors.map((elem) => {
  let primary = ["red", "yellow", "blue"];
  let isPrimary = primary.includes(elem);
  return {
    color: elem,
    isPrimary: isPrimary,
  };
...
const identifier = (arr) => {
  const filter = arr.filter((el) => el.programmer);
  return filter[0];
};
