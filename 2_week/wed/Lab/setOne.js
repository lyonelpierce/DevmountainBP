...
const findGrape = (arr) =>
  console.log(
    `The fruit with index ${arr.findIndex(
      (fruit) => fruit.color === "purple"
    )} is a grape`
  );
...
// CODE HERE
const oneLineFunction = (parameter1, parameter2) =>
  console.log(
...
function pricesPlusTax(prices, cb) {
  for (i = 0; i < prices.length; i++) {
    totalCost.push(prices[i] + prices[i] * 0.2);
  }
  cb(totalCost);
}
