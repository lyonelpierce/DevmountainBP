...
let reverseButtonColor = () => {
  sayHelloButton.style.backgroundColor = "";
  sayHelloButton.style.color = "";
};

sayHelloButton.addEventListener("mouseout", reverseButtonColor);
...
const repeatMyParam = (param) => {
  //YOUR CODE HERE
  axios.get(`http://localhost:3000/repeat/${param}`).then((res) => {
    console.log(res.data);
...
let getRequest = () => {
  axios.get("http://localhost:3000/query-test?name=Lyonel").then((res) => {
    console.log(res.data);
  });
};
...
/*
    ...
    YOU SEND AN EMPTY QUERY

    2: Send more than 1 query on the URL -- what happened? 
    YOU SENT MORE THATN 1 QUERY
*/
